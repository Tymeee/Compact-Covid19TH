...
struct Provider: TimelineProvider {
    func placeholder(in context: Context) -> Model {
        let loadingData = Model(date: Date(),covidData: CovidData(Confirmed: 0, Recovered: 0, Hospitalized: 0, Deaths: 0, NewConfirmed: 0, NewRecovered: 0, active: 0, NewDeaths: 0, NewHospitalized: 0, UpdateDate: ""))
        
        return loadingData
    }
    
    func getSnapshot(in context: Context, completion: @escaping (Model) -> ()) {
        let entry = Model(date: Date(),covidData: CovidData(Confirmed: 0, Recovered: 0, Hospitalized: 0, Deaths: 0, NewConfirmed: 0, NewRecovered: 0, active: 0, NewDeaths: 0, NewHospitalized: 0, UpdateDate: "01/01/2021 12:00"))
        completion(entry)
    }
